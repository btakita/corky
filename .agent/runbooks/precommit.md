...
3. **Audit instruction files** (if changed)
   ...
   - If OAuth desktop notification behavior changed, verify the docs mention `notify-desktop` on Linux, `osascript` on macOS, and PowerShell NotifyIcon on Windows
