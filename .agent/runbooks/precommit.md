...
3. **Audit instruction files** (if changed)
   ...
   - If OAuth desktop notification behavior changed, verify the docs mention `notify-desktop` on Linux with `notify-send` fallback, `osascript` on macOS, and PowerShell NotifyIcon on Windows
   ...
   - If Google Sheets tab sync changed, verify docs mention `corky sheets pull` / `corky sheets push`, tab creation, clear-before-write semantics, and `corky sheets delete-tab` when cleanup is supported
