...
- **Google OAuth hardening**: Interactive OAuth flows now bind the loopback listener before browser launch, default Google-backed auth to the fixed `127.0.0.1:8484` redirect, require explicit opt-in before arbitrary loopback fallback, reuse the shared redirect for GSC, and raise a best-effort desktop notification when user action is required.
